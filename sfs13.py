...
peak_intervals = deque(maxlen=100)  # Store time differences between peaks
...
def calculate_peak_count(data):
    ...
    return peak_count, peak_times

def calculate_peak_intervals(peak_times):
    intervals = []
...
def update_plots():
    ...
    # Time between peaks graph
    plt.subplot(3, 1, 3)
    plt.plot(peak_intervals, 'r-')
    plt.xlabel('Peak')
...
try:
    ...
    while True:
        ...
        peak_count, peak_times = calculate_peak_count(pressures)
        peak_counts.append(peak_count)
        if len(peak_times) >= 2:
            intervals = calculate_peak_intervals(peak_times)
