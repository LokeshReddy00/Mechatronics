...
timestamps = deque(maxlen=60)  # Store timestamps for the last 60 seconds
...
try:
    ...
    while True:
        ...
        if len(timestamps) >= 2 and timestamps[1] - timestamps[0] >= timedelta(minutes=1):
            peak_count = calculate_peak_count(pressures)
            print("Minute:", timestamps[0].strftime('%Y-%m-%d %H:%M'), "Peak Count:", peak_count)
